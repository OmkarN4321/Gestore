<script>
	...
	const login = async () => {
		if (email && password) {
			let res = await fetch(`https://pear-swallow-cap.cyclic.app:5000/userLogin?email=${email}&password=${password}`);
			res = await res.json();
...
<div class="auth">
	<div class="login">
		...
		<div class="login-inputs">
			<input type="text" placeholder="EMAIL" class="login-input" bind:value={email} />
